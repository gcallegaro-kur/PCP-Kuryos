  ...
  function iniciar(opcoes) {
    ...
    function render() {
      ...
      lista.innerHTML = pendentes.map(function(x) {
        ...
        return '<article class="agenda-pa-item"><div><b>'+e(a.cliente)+'</b><div>'+e(a.dataAgendada)+' '+e(a.janela)+' · '+(a.paletes||[]).length+' palete(s) · '+total.toLocaleString('pt-BR')+' un</div><div class="sub">Pedido(s) '+e(Object.values(a.pedidos||{}).join(', '))+'</div><div class="sub">'+e([a.transportadora||'Transportadora a definir',a.motorista||'Motorista a definir',a.placa||'Placa a definir'].join(' · '))+(a.contatoCliente?'<div class="sub">Recebimento: '+e([a.contatoCliente.nome,a.contatoCliente.telefone,a.contatoCliente.email].filter(Boolean).join(' · '))+'</div>':'')+'</div></div>'+
