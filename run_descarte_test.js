...
const db = { ref(path) { return {
  ...
  transaction(fn) { const next = fn(get(path)); if (next === undefined) return Promise.resolve({ committed: false, snapshot: { val: () => get(path), exists: () => get(path) != null } }); set(path, next); return Promise.resolve({ committed: true, snapshot: { val: () => get(path), exists: () => get(path) != null } }); },
