...
(async()=>{
  ...
  try{
    ...
    await context.exposeFunction('serverPA',async(name,dados)=>{
      ...
      for(const p of pages)await p.evaluate(state=>{for(const [no,cbs]of Object.entries(window.listenersPA||{}))for(const cb of cbs)cb({val:()=>structuredClone(state[no]||{})});},state);
      return {data:result};
    });
    await context.addInitScript(state=>{
      ...
      window.dbOnValue=(ref,cb)=>{(window.listenersPA[ref.path]||= []).push(cb);cb({val:()=>structuredClone(state[ref.path]||{})});};
    },state);
    ...
    for(const p of pages)await p.evaluate(clientes=>{for(const cb of window.listenersPA.clientes)cb({val:()=>clientes});},state.clientes);
